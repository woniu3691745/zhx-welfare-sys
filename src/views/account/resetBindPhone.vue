 ...
 */
<template>
  <div class="resetBindPhone" id="resetBindPhone">
    ...
    <div class="body">
      <div class="common-form-filed">
        <mt-field type="tel" label="手机号" placeholder="请输入新手机号" v-model="resetForm.phoneNum" class="book">
    ...
    <div class="bottom">
      <mt-button class="index-login" type="primary" @click="confirmClick">确认</mt-button>
    </div>
      </div>
  </div>
</template>
...
export default {
  ...
  methods: {
    ...
    // 密码
    passWordReg () {
      let {newLoginPassWord} = this.resetForm
    ...
    confirmClick () {
      ...
      if (!this.phoneReg() || !this.VerificationCode() || !this.passWordReg()) {
        ...
        let BINDPHONECORE = this.updatedpaypassword.BINDPHONECORE
        if (BINDPHONECORE == null) {
          me.alerts('请获取验证码')
          return false
        }
        let {phoneNum, identifyingCode, newLoginPassWord} = this.resetForm
        const data = {
          bizData: {
            Captacha: {
              PhoneNo: phoneNum,
